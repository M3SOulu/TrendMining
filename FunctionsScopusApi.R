...
get_scopus_papers_per_year <- function (search_string, domain="software", years = (2000:2017)){
...
get_scopus_papers <- function (query_string){
  ...
  while(found_items_num > 0){
    ...
    if (first_round){
      found_items_num <- as.numeric(resp$content$`search-results`$`opensearch:totalResults`)
      first_round=FALSE
    ...
    if (found_items_num==0){
      ...
      for(entry in entries){
        ...
        # print (entry$`prism:publicationName`)
        # print (entry$`prism:coverDate`)
        #print (entry$`author`)
        # print (i)
